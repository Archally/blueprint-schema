...
| `inprocess-consume-without-provide` | warn | an operation consumed in-process is provided in-process by some service. A consumer naming something nothing owns leaves the pair unjoined, so no dependency is drawn between the two contexts |
| `inprocess-provide-without-consume` | warn | an operation provided in-process is consumed in-process by some service. Either the caller is unmodelled, or the operation is reached some other way and the contract states a coupling that is not there |
| `shared-document-read-without-writer` | warn | a document read out of a shared store or a scheduled transfer is written by some service. The reader is bound to a schema the model does not say anyone produces |
...
| `fitness-function-unattached` | warn | a fitness function is named by something. It is a statement about the whole model and is legal on its own, so this is a recommendation rather than a defect: one named by a leverage point carries the reason it is enforced, one named by nothing states a rule whose motive lives outside the model |
