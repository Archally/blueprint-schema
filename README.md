...
**Capture blueprint knowledge without reading the schema** — the [**Blueprint Handoff Atlas**](docs/handoff-guides/markdown/README.md) explains how non-technical contributors should describe entities, relations, processes, screens, risks, and other layer knowledge so a technical person or AI agent can transform it into YAML. For workshop/offline use, see the committed [PDF set](docs/handoff-guides/pdf/).
...
Ships with **57 built-in rules** (42 warn, 15 info):

`activity-steps-reverse-causal-edge` · `activity-steps-without-causal-edge` · `activity-without-entry-operation` · `aggregate-root-signals` · `contract-operation-missing-channel-address` · `contract-operation-missing-exchange` · `dangling-relation-endpoint` · `decision-asserted-without-evidence` · `dependency-direction-disagreement` · `dispatch-with-exchange` · `empty-context` · `environment-config-key-not-declared` · `environment-named-not-declared` · `event-transport-without-contract` · `events-with-produces` · `exchange-missing-payload` · `fitness-function-unattached` · `implementation-as-is-without-code-refs` · `implementation-to-be-with-code-refs` · `inprocess-consume-without-provide` · `inprocess-dispatch-without-provider` · `inprocess-provide-without-consume` · `leverage-point-no-address` · `leverage-point-no-strategic-intent` · `missing-causal-links` · `missing-exchange-binding` · `model-without-purpose` · `model-without-represents` · `nav-via-action-screen-disagrees` · `operation-outside-every-contract` · `org-interaction-diverges-from-architecture` · `org-interaction-underived` · `org-interaction-unverifiable` · `orphan-entities` · `party-parts-disagree` · `payload-model-unbound` · `payload-schema-unresolved` · `process-end-state-unreachable` · `restated-coupling` · `sent-event-without-receiver` · `service-system-is-an-organization` · `service-without-system` · `shared-document-read-without-writer` · `shared-document-write-without-reader` · `shared-store-without-common-resource` · `story-without-event` · `unanswered-questions` · `unbound-operation` · `unbound-question` · `undeclared-exchange` · `undescribed-event` · `unevidenced-dependency` · `untested-rules` · `use-case-step-action-operation-disagrees` · `use-case-step-action-screen-disagrees` · `user-story-use-case-back-reference` · `user-story-without-acceptance-criteria`
...
Non-technical handoff guides for capturing Blueprint knowledge in plain language before it is transformed into YAML.

```bash
npm run guides:pdf      # regenerate docs/handoff-guides/pdf/**
npm run guides:check    # drift check for committed PDFs
```

Start with the [umbrella guide](docs/handoff-guides/markdown/README.md), the [umbrella PDF](docs/handoff-guides/pdf/blueprint-handoff-atlas.pdf), or browse the full [PDF directory](docs/handoff-guides/pdf/).
