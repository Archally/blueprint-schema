...
Ships with **50 built-in rules** (36 warn, 14 info):

`activity-steps-reverse-causal-edge` · `activity-steps-without-causal-edge` · `activity-without-entry-operation` · `aggregate-root-signals` · `contract-operation-missing-channel-address` · `contract-operation-missing-exchange` · `decision-asserted-without-evidence` · `dependency-direction-disagreement` · `dispatch-with-exchange` · `empty-context` · `environment-config-key-not-declared` · `environment-named-not-declared` · `event-transport-without-contract` · `events-with-produces` · `exchange-missing-payload` · `fitness-function-unattached` · `implementation-as-is-without-code-refs` · `implementation-to-be-with-code-refs` · `inprocess-consume-without-provide` · `inprocess-dispatch-without-provider` · `inprocess-provide-without-consume` · `leverage-point-no-address` · `leverage-point-no-strategic-intent` · `missing-causal-links` · `missing-exchange-binding` · `model-without-purpose` · `model-without-represents` · `org-interaction-diverges-from-architecture` · `org-interaction-underived` · `org-interaction-unverifiable` · `orphan-entities` · `party-parts-disagree` · `payload-model-unbound` · `payload-schema-unresolved` · `restated-coupling` · `service-system-is-an-organization` · `service-without-system` · `shared-document-read-without-writer` · `shared-document-write-without-reader` · `shared-store-without-common-resource` · `story-without-event` · `unanswered-questions` · `unbound-operation` · `unbound-question` · `undeclared-exchange` · `undescribed-event` · `unevidenced-dependency` · `untested-rules` · `user-story-use-case-back-reference` · `user-story-without-acceptance-criteria`
